...
if __name__  == "__main__":
    ...
    # ！Here need to introduce a new function to get the intrinsic matrix for outside input
    # Define the intrinsic matrix K0 for the prompt image
    K0 = np.array(
    ...
    # Define the intrinsic matrix K1 for the target image
    K1 = np.array(
    ...
    # Define the dimensions of the 3D bounding box
    ...
    # Generate masks for the target image
    t1 = time.time()
    masks = MASK_GEN.generate(target_image)
    t2 = time.time()
    
    # Initialize similarity scores and top images
    similarity_score, top_images = np.array([0, 0, 0], np.float32), [[], [], []]
    t3 = time.time()
    
    # Define the compact percent for bounding box adjustment
    compact_percent = 0.3
    
    # Iterate through each mask
    for _, mask in enumerate(masks):
        # Extract the object mask and bounding box
        object_mask = np.expand_dims(mask["segmentation"], -1)
        x0, y0, w, h = mask["bbox"]
        x1, y1 = x0 + w, y0 + h
        
        # Adjust the bounding box with compact percent
        ...
        # Resize the image crop and get the intrinsic matrix for the crop
        ...
        # Further resize the image crop to 256x256
        ...
        # Transform the cropped image to the form expected by the model
        crop_tensor = set_torch_image(image_crop, center_crop=True)
        
        # Get the feature vector for the cropped image
        with torch.no_grad():
            fea = get_cls_token_torch(dinov2_model, crop_tensor)
        
        # Compute the cosine similarity score between the reference and cropped image features
        score = F.cosine_similarity(ref_fea, fea, dim=1, eps=1e-8)
        
        # Update the top images if the current score is higher
    ...
    # Load the prompt and target poses (extrinsics) from text files
